...
func (msg *MsgPayForMessage) ValidateBasic() error {
	...
	if nsLen := len(msg.GetMessageNamespaceId()); nsLen != consts.NamespaceSize {
		return fmt.Errorf(
...
func CreateCommitment(k uint64, namespace, message []byte) ([]byte, error) {
	...
	for i, set := range leafSets {
		...
		tree := nmt.New(sha256.New(), nmt.NamespaceIDSize(consts.NamespaceSize))
...
func chunkMessage(message []byte) [][]byte {
	var shares [][]byte
	for i := 0; i < len(message); i += consts.ShareSize {
		end := i + consts.ShareSize
		if end > len(message) {
			end = len(message)
		}
...
func padMessage(msg []byte) []byte {
	...
	if uint64(len(msg))%consts.ShareSize == 0 {
		return msg
	}
